class ProducesController < ApplicationController
  ...
  # GET /produces.json
  def index
    @produces = Farmer.find_by_user_id(current_user.id).produces
  end
  ...
  def create
    ...
    respond_to do |format|
      if @produce.save
        format.html { redirect_to produces_path, notice: 'Produce was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @produce.update(produce_params)
        format.html { redirect_to produces_path, notice: 'Produce was successfully updated.' }
  ...
  private
    ...
    def produce_params
      params.require(:produce).permit(:name, :description, :available, :farmer_id, :images => [])
    end
end
